...
mybatis.type-aliases-package=com.fishx.dao

#指定 mapper.xml 的路径 (application 上配置了@MapperScan(扫描mapper 类的路径)和 pom.xml 中放行了 mapper.xml 后
#配置mapper-locations 没有意义. 如果mapper类和mapper.xml不在同一个路径下时，mapper-locations 就有用了) 
...
#日志打印:日志级别 trace < debug < info < warn < error < fatal 
#默认级别为 info，即默认打印 info 及其以 上级别的日志
#logging.level 设置日志级别，后面跟生效的区域
#比如 root 表示整个项目，也可以设置为某个包下，也可以具体到某个类名（日志级别的值不区分大小写）
...
#redis 连接配置 
# redis 所在主机 ip 地址 
spring.redis.host=127.0.0.1:xxxx
#redis 服务器密码
spring.redis.password=123456
#redis 服务器端口号 
spring.redis.port=127.0.0.1
